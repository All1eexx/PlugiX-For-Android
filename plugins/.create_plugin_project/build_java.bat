...
set "TEMPLATE_DIR=%SCRIPT_DIR%\templates\java"
...
for /f "usebackq tokens=1* delims==" %%A in (`
    findstr /b /r /c:"android_sdk=" /c:"android_ndk=" /c:"java_home=" /c:"d8=" /c:"default_abi=" /c:"min_sdk=" /c:"build_tools_version=" /c:"platform_version=" "%CONFIG_FILE%"
`) do (
    set "key=%%A"
    set "val=%%B"
    set "!key!=!val!"
...
for %%F in (build_PLUGINNAME.bat PLUGINNAME.java) do (
    if exist "%%F" (
        set "oldname=%%F"
...
for %%F in (build_%PROJECT_NAME%.bat %PROJECT_NAME%.java) do (
    if exist "%%F" (
...
echo Java project "%PROJECT_NAME%" created at "%TARGET_DIR%"
exit /b 0
