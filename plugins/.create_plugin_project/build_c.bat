...
set "TEMPLATE_DIR=%SCRIPT_DIR%\templates\c"
...
for %%F in (build_PLUGINNAME.bat PLUGINNAME.c PLUGINNAME.h) do (
    if exist "%%F" (
        set "oldname=%%F"
...
for %%F in (build_%PROJECT_NAME%.bat %PROJECT_NAME%.c %PROJECT_NAME%.h CMakeLists.txt) do (
    if exist "%%F" (
